# Sharath Chandra — Portfolio Maintenance Guide
...
| --- | --- |
| Name, email, phone, GitHub, LinkedIn | `app/content.ts` → `profile` |
...
These values are reused in the navigation, social links, contact section, email buttons, and phone button.

Important:

- Keep complete URLs beginning with `https://`.
- Keep the phone number inside quotes.
...
- [ ] Email, phone, GitHub, and LinkedIn links are correct.
...
Then ask Codex to publish the updated portfolio. A useful request is:

> Update my existing portfolio from the current files, verify the build, and publish it to the same live URL.
